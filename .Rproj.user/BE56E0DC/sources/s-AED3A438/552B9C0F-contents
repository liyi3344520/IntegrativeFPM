...
#'
#' @param cv_a is a flag for using cross validation for choosing lambda_a
...
#'
#' @param cv_b is a flag for using cross validation for choosing lambda_b
#'
#'
#' @return
#' \itemize{
...
IntegrativeFPM <- function(y, x, deltaB, sw, family, lambda_a, cv_a, lambda_b, cv_b){
  x.AB<-x
  y.AB<-y
  ...
  beta.ini <-rep(0,p+1)
  if(cv_a==1){
    grid1<-exp(seq(-1.5, 1, 0.1))
    grid1<-rev(grid1)
    grid1
    lambda_a<-cv.ee_a(deltaB,x.AB,y.AB,sw,nA,nB,alpha.ini,grid1)
    lambda_a
  }
  par0<-c(alpha.ini)
  En<-matrix(0,p+1,p+1)
  ...
  if(family=="gaussian"){
    ...
    if(cv_b==1){
      beta <- ncvreg::cv.ncvreg(x.B, y.B, penalty='SCAD', family="gaussian")
      beta.est <- coef(beta, s='lambda.min')
      beta.est <- beta.est[1:(p+1)]
    }
    if(cv_b==0){
  ...
  if(family=="binomial"){
    ...
    if(cv_b==1){
      beta <- ncvreg::cv.ncvreg(x.B, y.B, penalty='SCAD', family="binomial")
      beta.est <- coef(beta, s='lambda.min')
      beta.est <- beta.est[1:(p+1)]
    }
    if(cv_b==0){
...
cv.ee_a<-function(deltaB,x.AB,y.AB,sw,nA,nB,alpha.ini,grid1,Kfold=3){
  ## CV tuning par
  ## creating number=Kfold folds
  n1<-ceiling(nA/Kfold)
  foldA<-rep(1:Kfold,n1)
  foldA<-foldA[1:nA]
  n2<-ceiling(nB/Kfold)
  foldB<-rep(1:Kfold,n2)
  foldB<-foldA[1:nB]
  fold<-c(foldA,foldB)
  KK<-length(grid1)
  p<-dim(x.AB)[2]
  ...
  for(kk in 1:KK){
    ...
    addloss<-0
    for(ff in 1:Kfold){
      train.id<-which(fold!=ff)
      test.id<-which(fold==ff)
      thisdeltaB<-deltaB[train.id]
      thisx.AB<-x.AB[train.id,]
      thisy.AB<-y.AB[train.id]
      thissw<-sw[train.id]
      thatdeltaB<-deltaB[test.id]
      thatx.AB<-x.AB[test.id,]
      thaty.AB<-y.AB[test.id]
      thatsw<-sw[test.id]
      par0<-c(alpha.ini)
      En<-matrix(0,p+1,p+1)
      cnt<-0
      ...
      for(jj in 1:100){
        cnt<-cnt+1
        Uee0<-Uee_a(par0,thisdeltaB,thisx.AB,thisy.AB,thissw)
        Aee0<-Aee_a(par0,thisdeltaB,thisx.AB,thisy.AB,thissw)
        diag(En)<-abs(q1(par0,lambda))/(epsilon+abs(par0))
        par<-  par0+ ginv(Aee0+En)%*%(Uee0-En%*%par0)
        if( sum( abs(par-par0)  )<10^(-6) )break;
        if( sum( abs(par-par0)  )>1000 )break;
        par0<-par
      }
      addloss<-addloss+loss_alpha(par,thatdeltaB,thatx.AB,thaty.AB,thatsw)
    }
    cv1[kk]<-addloss
    if(kk>1){
      if(cv1[kk]>cv1[kk-1])break;
    }
  }
  lambda_a<-grid1[which.min(cv1)]
  lambda_a
}
